...
from .tasks import send_student_extended_reports_if_complite
...
def _listen_for_completed_course(sender, user, course_id, **kwargs):
    send_student_extended_reports_if_complite.delay(user.id, str(course_id))
...
def _listen_for_video_lesson_update(sender, user, course_id, **kwargs):
    send_student_extended_reports_if_complite.delay(user.id, course_id)
