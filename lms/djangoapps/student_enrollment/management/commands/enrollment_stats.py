...
class FiveDayEnrollmentReport(BaseCommand):
    ...
    def get_statistics(module_locator):
        """
        ...
        module_enrollment_records = CourseEnrollment.objects.filter(
            course_id=module_locator)
        
        # Set the default values
        self.number_of_students_enrolled = len(module_enrollment_records)
        self.number_of_students_logged_in = 0

        for enrollment in module_enrollment_record:
            if User.objects.get(email=enrollment.user).last_login is not None:
                number_of_students_logged_in += 1
        
        self.total_percentage = 100 * float(
            number_of_students_logged_in) / float(number_of_students_enrolled)
