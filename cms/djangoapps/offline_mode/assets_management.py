...
def get_static_file_path(relative_path):
    ...
    """
    base_path = settings.STATIC_ROOT  # /openedx/staticfiles/studio
    ...
    try:
        if filename.startswith("assets/"):
            ...
            asset_key = StaticContent.get_asset_key_from_path(
                xblock.location.course_key, asset_filename
            )
...
def is_modified(xblock):
    ...
    file_path = os.path.join(
        block_storage_path(xblock), f"{xblock.location.block_id}.zip"
    )
